...
def collect_courses() -> list[Item]:
    items: list[Item] = []
    for base in [ROOT / "learning" / "courses", ROOT / "learning" / "education"]:
        if not base.exists():
            continue
        ...
        for project_dir in sorted([p for p in base.iterdir() if p.is_dir()]):
            ...
            items.append(
                Item(
                    ...
                    status=normalize_status(extract_field(text, "Status")) if text else default_status,
...
def summarize(items: list[Item]) -> tuple[int, int, int, int, int]:
    total = len(items)
    done = sum(1 for i in items if i.status == "done")
    in_progress = sum(1 for i in items if i.status == "in-progress")
    planned = total - done - in_progress
    completion = int(round((done / total) * 100)) if total else 0
    return total, done, in_progress, planned, completion
...
def render_table(items: list[Item], include_next_step: bool = False) -> str:
    ...
    if not rows:
        rows = ["| _No entries yet_ | n/a | n/a | n/a |" if include_next_step else "| _No entries yet_ | n/a | n/a | n/a |"]
    return "\n".join([header, *rows])
...
def main() -> None:
    ...
    c_total, c_done, c_in_progress, c_planned, c_completion = summarize(courses)
    p_total, p_done, p_in_progress, p_planned, p_completion = summarize(projects)
    ...
    content = "\n".join(
        [
            "# Progress Tracker",
            "",
            f"_Auto-generated by `scripts/update_progress_tracker.py` on {generated_at}._",
            "",
            "## Summary",
            "",
            "| Area | Total | Done | In Progress | Planned/Unknown | Completion |",
            "|---|---:|---:|---:|---:|---:|",
            f"| Courses | {c_total} | {c_done} | {c_in_progress} | {c_planned} | {c_completion}% |",
            f"| Home Projects | {p_total} | {p_done} | {p_in_progress} | {p_planned} | {p_completion}% |",
            "",
            "## Courses",
            "",
